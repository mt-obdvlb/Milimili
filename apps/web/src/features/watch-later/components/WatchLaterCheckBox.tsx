...
const WatchLaterCheckBox = ({
  ...
  const handleChange = (value: boolean) => {
    if (value) {
      ...
      setIds((prev) => prev.filter((item) => item.videoId !== id.videoId))
    }
  }
  const check = useMemo(
    () => ids.some((item) => item.videoId === id.videoId),
  ...
  return (
    ...
    >
      <div
        className={cn(
          'absolute top-2 left-2 size-[20px] rounded-[3px] border-[1.5px] border-white bg-[rgba(0,0,0,.2)] text-white opacity-100',
          check && 'border-brand_pink bg-brand_pink'
        )}
      >
        <div
          className={cn(
            'pointer-events-none flex size-full items-center justify-center rounded-[3px] opacity-0',
            check && 'opacity-100'
          )}
        >
          <svg
