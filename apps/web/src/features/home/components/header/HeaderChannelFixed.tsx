...
const HeaderChannelFixed = ({ categoryList }: { categoryList?: CategoryGetAllList }) => {
  ...
  const layoutStyles = tv({
    slots: {
      base: 'group m-0 mx-auto relative h-[56px] flex hover:h-[140px] w-full items-center overflow-hidden px-[60px] py-[14px] transition-[height] duration-200',
      left: 'text-text2 flex w-[140px] min-w-[140px] flex-wrap self-start text-sm',
      center: 'bg-line_regular mx-0 my-0 mr-2.5 h-[14px]',
      right: 'relative inline-grid h-[100%] flex-1 grid-cols-[repeat(14,1fr)] gap-x-2.5 gap-y-3.5',
      arrow:
        'absolute group-hover:rotate-180 right-[calc(60px-40px)] ml-2.5 flex size-7 cursor-pointer items-center self-start rounded-[4px] p-2 transition duration-200',
    },
  })

  const { base, arrow, left, right, center } = layoutStyles()
  ...
  return (
    ...
    >
      <div className={cn(base())}>
          ...
          {[
            {
              ...
              svg: (
                <svg
                  ...
                  className={'mr-[5px] h-[23px] w-[22px] stroke-[currentColor] text-inherit'}
                >
                  <path
            ...
            },
            {
              ...
              svg: (
                <svg
                  ...
                  fill='none'
                  xmlns='http://www.w3.org/2000/svg'
                  className={'mr-[5px] size-[22px] text-inherit'}
                >
                  <path
                    fillRule='evenodd'
                    clipRule='evenodd'
        ...
        <Separator orientation='vertical' className={center()} />
        <div className={right()}>
          <div
            className={
              'relative col-span-14 inline-grid grid-flow-row grid-cols-14 gap-x-2.5 gap-y-3.5'
            }
          >
              ...
              <Link href={`/category/${item.id}`} key={item.id}>
                <div
                  className={cn(
                    ...
                    item.name.length > 3 && 'tracking-normal'
                  )}
                >
                  {item.name}
                </div>
