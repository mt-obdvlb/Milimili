...
module Display {
  class DirtyArea {
    ...
    public isEmpty(): boolean {
      return (
        this._xBegin === null || this._yBegin === null || this._xEnd === null || this._yEnd === null
      )
    }
  ...
  export class BitmapData {
    ...
    private _updateBox(changeRect: Rectangle = null): void {
      ...
      var change: Rectangle = changeRect === null ? this._dirtyArea.asRect() : changeRect

      // Make sure we're not out-of-bounds
      if (!this._rect.containsRect(change)) {
        __trace(
          'BitmapData._updateBox box ' +
            change.toString() +
            ' out of bonunds ' +
            this._rect.toString(),
          'err'
      ...
      for (var i = 0; i < change.height; i++) {
        for (var j = 0; j < change.width; j++) {
          region.push(this._byteArray[(change.y + i) * this._rect.width + change.x + j])
        }
      }
    ...
    public getPixel32(x: number, y: number): number {
      if (x >= this._rect.width || y >= this._rect.height || x < 0 || y < 0) {
        throw new Error('Coordinates out of bounds')
      }
      try {
        return this._transparent
          ? this._byteArray[y * this._rect.width + x]
      ...
      for (var i = 0; i < rect.height; i++) {
        Array.prototype.push.apply(
          ...
          this._byteArray.slice(
            (rect.y + i) * this._rect.width + rect.x,
      ...
      if (input.length !== rect.width * rect.height) {
        throw new Error(
          'setPixels expected ' +
            ...
            ' pixels, but actually got ' +
            input.length
        )
      }
      for (var i = 0; i < rect.width; i++) {
        for (var j = 0; j < rect.height; j++) {
          this._byteArray[(rect.y + j) * this.width + (rect.x + i)] = input[j * rect.width + i]
