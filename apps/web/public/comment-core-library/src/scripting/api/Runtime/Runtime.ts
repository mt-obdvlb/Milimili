...
module Runtime {
  ...
  export interface Listenable {
    ...
    removeEventListener(event: string, listener: Function, useCapture: boolean): void
  ...
  export function generateId(type: string = 'obj'): string {
    var id: string = [
      ...
      Date.now(),
      '|',
      Runtime.NotCrypto.random(16),
      ':',
      objCount,
    ].join()
    while (Runtime.hasObject(id)) {
      id = type + ':' + Date.now() + '|' + Runtime.NotCrypto.random(16) + ':' + objCount
    }
    return id
  }
