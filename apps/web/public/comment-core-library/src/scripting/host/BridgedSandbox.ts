...
export class BridgedSandbox implements ISandbox {
  ...
  private _bind(): void {
    ...
    this._worker.setMalformedEventHandler(
      ((e) => {
        if (e.stack) {
    ...
    this._worker.setDefaultChannelListener(
      ((m: OOAPIMessage) => {
        this._scripter.logger.warn('Got message for non-existant channel "' + m.channel + '".')
      }).bind(this)
    )

    // Add a channel for logging
    this._worker.addChannelListener(
      '',
      ((m: OOAPIMessage) => {
        // Log message
