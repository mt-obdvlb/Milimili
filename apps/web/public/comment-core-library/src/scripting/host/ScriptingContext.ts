  ...
  class BaseScriptingContext implements IScriptingContext {
    ...
    public registerObject(objectId: string, serialized: ISerializedData): void {
      if (this.hasObject(objectId)) {
        throw new ScriptingContextError(
          'Attempting to register object (' + objectId + ') but object already present.'
        )
      }
      var obj = new StageObject(this.stage, Unpacker.unpack(serialized.class, serialized))
      obj.construct()
    ...
    public callMethod(objectId: string, methodName: string, parameters: any[]): void {
      ...
      if (typeof obj[methodName] !== 'function') {
        throw new ScriptingContextError(
          'Object(' + objectId + ') does not have method "' + methodName + '".'
        )
      } else {
